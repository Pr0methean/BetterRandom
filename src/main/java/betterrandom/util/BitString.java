...
/**
 * <p>Implementation of a fixed-length bit-string.  Internally, bits are packed into an array of
 * ints.  This implementation makes more efficient use of space than the alternative approach of
 * using an array of booleans.</p> <p>This class is preferable to {@link java.util.BitSet} if a
...
public final class BitString implements Cloneable, Serializable {
  ...
  /**
   * Store the bits packed in an array of 32-bit ints.  This field cannot be declared final because
   * it must be cloneable.
   */
  private int[] data;
   ...
   * bits is uniform so long as the output from the provided RNG is also uniform).  Using this
   * constructor is more efficient than creating a bit string and then randomly setting each bit
   * individually.
   *
   * @param length The number of bits.
   ...
   * enough that they contain the full length of the specified substring.
   *
