...
protected constructor(seed: ByteArray?) : Random(), ByteArrayReseedableRandom, RepeatableRandom, Dumpable, EntropyCountingRandom {
    ...
    protected val entropyBits = AtomicLong(0)
     ...
     */
    protected var seed: ByteArray? = null
    /**
    ...
    protected open fun setSeedInternal(seed: ByteArray?) {
        if (this.seed == null || this.seed!!.size != seed!!.size) {
            ...
            System.arraycopy(seed, 0, this.seed!!, 0, seed.size)
        }
    ...
    open fun creditEntropyForNewSeed(seedLength: Int) {
        entropyBits.updateAndGet { oldCount -> Math.max(oldCount, Math.min(seedLength, newSeedLength) * 8L) }
    }
    ...
    protected fun fallbackSetSeed() {
        if (seed == null) {
            seed = DefaultSeedGenerator.DEFAULT_SEED_GENERATOR.generateSeed(newSeedLength)
        }
    }
