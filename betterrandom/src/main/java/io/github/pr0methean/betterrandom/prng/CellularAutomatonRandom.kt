...
import io.github.pr0methean.betterrandom.util.BinaryUtils.Companion.convertIntToBytes

/**
 * Java port of the [cellular automaton pseudorandom number generator](http://home.southernct.edu/~pasqualonia1/ca/report.html) developed by Tony Pasqualoni.
 *
 * @author Tony Pasqualoni (original C version)
 * @author Daniel Dyer (Java port)
 */
class CellularAutomatonRandom : BaseRandom {
    private var cells: IntArray? = null
     ...
     */
    constructor(seed: Int) : super(convertIntToBytes(seed))
    ...
    override fun setSeed(seed: Long) {
        val shortenedSeed = convertIntToBytes(seed.hashCode())
        if (superConstructorFinished) {
            ...
            this.seed_ = shortenedSeed // can't do anything else yet
        }
    }
    ...
    override fun setSeedInternal(seed: ByteArray?) {
        ...
        try {
            ...
            if (cells == null) {
                cells = IntArray(AUTOMATON_LENGTH)
            }
            ...
            for (i in 0 until AUTOMATON_LENGTH - 4) {
                cells!![i] = 0x000000FF and (seedAsInt shr i % 32)
            }
            // Evolve automaton before returning integers.
            for (i in 0 until EVOLVE_ITERATIONS_AFTER_SEEDING) {
                internalNext()
            }
        } finally {
            if (locked) {
