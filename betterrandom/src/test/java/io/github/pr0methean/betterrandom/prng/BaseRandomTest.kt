...
abstract class BaseRandomTest : PowerMockTestCase() {
    ...
    open fun testRepeatability() {
        ...
        val duplicateRNG = createRng(rng.getSeed())
    ...
    open fun testReseeding() {
        ...
        rng1.setSeed(testSeedGenerator.generateSeed(seedLength))
    ...
    open fun testRandomSeederThreadIntegration() {
        ...
        while (rng.getEntropyBits() > java.lang.Long.SIZE) {
            rng.nextLong()
        }
    ...
    fun testWithProbability() {
        val prng = createRng()
        val originalEntropy = prng.getEntropyBits()
    ...
    companion object {
        ...
        protected val SET_SEED = NamedFunction<Random, Double>({ random ->
            if (random is BaseRandom) {
                ...
                baseRandom.setSeed(SEMIFAKE_SEED_GENERATOR.generateSeed(baseRandom.newSeedLength))
            } else {
