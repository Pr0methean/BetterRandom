...
if ( $STATUS ) {
    if ( $env:TRAVIS ) {
        ...
        git config --global user.email "appveyor@appveyor.com" *>&1
    } else {
    # Not in CI
        $COMMIT = git rev-parse HEAD
        $JOB_ID = [guid]::NewGuid()
    }
    GIT_CURL_VERBOSE=1 git clone "https://github.com/Pr0methean/betterrandom-coverage.git" *>&1
    ...
    if ( $TRAVIS ) {
        ...
        git config --global user.email "travis@travis-ci.org" *>&1
    }
    if ( ! $JAVA9 ) {
